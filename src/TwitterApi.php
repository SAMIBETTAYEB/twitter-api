...
class TwitterApi
{
    ...
    public function call($method = 'GET', $url = '', $headers = [], $postFields = [], $basic = false, $username = '', $password = '')
    {
        ...
        dump('--------- START response ---------', $response, '------- END response --------');
    ...
    public function logUser()
    {
        ...
        //Cache::forever('Authorization', $httpHeaders);
        // dd($httpHeaders);
    ...
    public function testAuth($accessToken, $accessTokenSecret)
    {
    ...
    public function getUserFavorites($accessToken, $accessTokenSecret, $username, $max_id = '')
    {
        ...
        if (count($favorites) > 1) {
            dump(count($favorites), $favorites);
            sleep(12);
